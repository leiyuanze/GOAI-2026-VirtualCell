...
| **最终：场景自适应** | **chem用v2.1集成+其余用v3.7** | 0.878 | **0.676** | **0.760** | 0.829 |

> **最终方案：场景自适应提交**。评测与 test 均按 split_final 分四个场景，每个场景用各自最强的模型：test_chem_only 用 3×v2.1 集成（val_chem 0.878），test_strain_only/test_both/test_time 用 v3.7 单模型（val_strain 0.676 / val_both 0.760 / val_time 0.829）。相比交接时的 3×v2.1+v2.5 集成，**val_strain +0.020、val_both +0.016，val_chem/val_time 仅 -0.001/-0.002**——核心难点（unseen 菌株）大幅提升，其余场景几乎持平。
>
> **核心洞察**：单模型与集成各有优势且不可兼得（单模型 val_strain 强、集成 val_chem 强），但评测是分场景的，场景自适应可让每个场景都用最强模型，绕过这个权衡。
>
> **GO 通路注意力（v3.7 核心，对应教程 5.4.4 蛋白侧结构化先验「通路归属」）**：从 UniProt GO 生物过程注释构建 92 个高频通路的「通路→蛋白」矩阵，模型输出叠加「通路效应 @ 通路矩阵」——同一通路蛋白共享权重偏置，实现"同通路蛋白协同响应"的输出空间结构化。这是本方案最大的单项提升（val_strain +0.013、val_both +0.010）。
