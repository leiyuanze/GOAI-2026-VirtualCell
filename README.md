...
│   ├── 08b_mig_fusion_v50ens.py   # 合规迁移融合（train-only 迁移池，8/16）
│   ├── 08c_mig_fusion_adaptive.py  # 自适应 α 迁移融合（最终版，val 验证 α 形式，8/16）
│   ├── 04_model_v50.py            # v5.0/v5.1 control+delta 低秩重构模型（8/16 评审路线，v5.1 加组件监督+RDKit desc）
