...
> **最终方案：v3.7 单模型**（不集成）。蛋白 R² 四场景 0.675~0.869，其中 val_strain 0.675、val_both 0.762 为 unseen 菌株场景（M4+M5 共 30% 权重）的历史最优。相比交接时的 3×v2.1+v2.5 集成（val_strain 0.656 / val_both 0.744），val_strain +0.019、val_both +0.018。
>
> **为什么不用集成**：教程 5.6.3 明确"集成不是必须，优先单模型"。实测 v2.1 三兄弟（val_strain 0.64）会把 v3.7 的 0.676 拉低到 0.667，集成反而损害 unseen 菌株场景。
>
