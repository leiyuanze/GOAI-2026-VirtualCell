...
| **场景自适应 v2 + 迁移融合** | **time→3×v2.1+v35，chem 新化合物加迁移** | 0.878 | **0.676** | **0.760** | 0.833 |
...
>
> **迁移融合对 val 集的影响**：迁移融合仅作用于 test 集的新化合物样本（11 个未见过化合物的 SMILES 由用户提供的 test 元数据派生），val 集里没有对应的新化合物样本，因此表中的 val 列与 v1 完全一致。迁移融合的实际收益在测试集上：test_chem_only 总分 0.505 → 0.512（+0.007）。
>
