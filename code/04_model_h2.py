...
class VCellModel(nn.Module):
    def __init__(self, feats, P=4422, hidden=256, latent=128, drop_ent=0.0):
        ...
        d_in = 16 + 32 + 32 + 2 + 1 + 3 + 4 + 8   # 66 维（去 hash）
    ...
    def _emb(self, x):
        ...
        return torch.cat([se, ce, chem_hash, med, temp, tfeat, sme, cte], dim=1)

    def _decompose(self, bio, chem_seen, strain_seen):
        h = self.enc(bio)
        b, s, c, t = self.fc_B(h), self.fc_S(h), self.fc_C(h), self.fc_T(h)
        base_c = 0.05 + 0.95 * chem_seen
        base_s = 0.05 + 0.95 * strain_seen
    ...
    def neutral_latent(self, x, neutral='strain'):
        ...
        chem_seen, strain_seen = x['seen']
        if neutral == 'strain':
            x2 = {'bio': [torch.full_like(strain_id, self.NEUTRAL_S), chem_id, chem_hash, med, temp, tfeat, sm_id, ct_id],
                  'ctx': x['ctx'], 'seen': [chem_seen, torch.zeros_like(strain_seen)]}
        else:
            x2 = {'bio': [strain_id, torch.full_like(chem_id, -1), torch.zeros_like(chem_hash), med, temp, tfeat, sm_id, ct_id],
