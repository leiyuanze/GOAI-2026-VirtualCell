# -*- coding: utf-8 -*-
"""
GOAI 虚拟细胞 · 04 模型定义（v2.5：输入级分离的残差分解）
修正 v2.1 的「伪解耦」：C/T 分支不再吃混合 latent，改为输入级分离——
  C 分支输入 = [chem_emb, chem_hash, 培养基, 温度, 时间]（不含菌株）
  T 分支输入 = [strain_emb, 培养基, 温度, 时间]（不含化合物）
架构级保证解耦（temp1/教程本意），配合 gate 可见性门控。
"""
import numpy as np
import torch
...
class VCellModel(nn.Module):
    def __init__(self, feats, P=4422, n_modules=64, hidden=256, latent=64):
        ...
        # T 分支编码：菌株侧 [strain_emb16 + med2 + temp1 + time3 + sm4] = 26
        d_strain = 16 + 2 + 1 + 3 + 4
        ...
        self.proj = nn.Linear(latent, P)   # 单层投影（v2.2 已证与模块头等价）
    ...
    def forward(self, x, with_components=False):
        ...
        hc = self.enc_C(bio_c)
        # 菌株侧（不含化合物）
        bio_t = torch.cat([se, med, temp, tfeat, sme], dim=1)
        ht = self.enc_T(bio_t)
