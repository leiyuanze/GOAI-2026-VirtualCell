# -*- coding: utf-8 -*-
"""
GOAI 虚拟细胞 · 04 模型定义（v2.9：交互项 + 自适应loss权重）
基于 v2.7，增加：
  1) 菌株×化合物显式交互项（对应 M5 双重未知 10%）
  2) 可学习的 loss 不确定度权重（Kendall et al. 2018 自适应多任务）
"""
import numpy as np
import torch
...
class VCellModel(nn.Module):
    def __init__(self, feats, P=4422, hidden=256, latent=64):
        ...
        # ★ 自适应 loss 权重（Kendall uncertainty）
