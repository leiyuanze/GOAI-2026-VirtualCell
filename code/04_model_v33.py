...
class VCellModel(nn.Module):
    ...
    def forward(self, x, with_components=False):
        ...
        strain_sim = x['strain_sim']  # (N, 4) 遗传相似度
        ...
        hc = self.enc_C(bio_c)
        bio_t = torch.cat([se, cp, med, temp, tfeat, sme, strain_sim], dim=1)
        ht = self.enc_T(bio_t)
        ...
        # ★ 遗传相似度部分激活 unseen 菌株的 T 分支（seen=1，unseen=最大遗传相似度）
        sim_max = strain_sim.max(dim=1, keepdim=True).values
        g_s = torch.sigmoid(self.gate_s) * (strain_seen.unsqueeze(1) + (1 - strain_seen.unsqueeze(1)) * sim_max)
        z = b + s + g_c * c + g_s * t + g_i * interact

        # ★ ESM2 通路（带门控）：wz @ esm2_emb.T 给每个蛋白一个序列先验贡献
        wz = self.w_esm(z)  # (N, 64)
        out = self.proj(z) + torch.sigmoid(self.gate_esm) * torch.mm(wz, self.esm2_emb.T) + self.bias
