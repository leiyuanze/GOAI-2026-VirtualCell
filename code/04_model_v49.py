# -*- coding: utf-8 -*-
"""
GOAI 虚拟细胞 · 04 模型定义（v3.7：GO通路注意力+蛋白先验）
...
class VCellModel(nn.Module):
    ...
    def forward(self, x, with_components=False):
        ...
        # ★ v4.9：unseen 化合物 C 分支门控改为可学习（原 0.2+0.8*seen 压制 80%）
        # 现 test 新化合物有真实指纹，模型可自行决定 unseen 时 C 分支的利用程度
        g_c = torch.sigmoid(self.gate_c) * (0.5 + 0.5 * chem_seen).unsqueeze(1)
        ...
        # ★ GO 通路注意力（带门控）：通路效应 @ go_mat，同一通路蛋白共享偏置
