# -*- coding: utf-8 -*-
"""
GOAI 虚拟细胞 · 05 训练 v3.3（T分支增强 + unseen门控归零）
...
def make_x(idx):
    return {
        ...
        'strain_sim': torch.from_numpy(feats['strain_sim'][idx]),
    }

# ---------- 模型 ----------
import importlib.util
_spec = importlib.util.spec_from_file_location("m33", r"D:\leiyuanze\Datawhale\AI for Research\虚拟细胞\vcell\04_model_v33.py")
_m33 = importlib.util.module_from_spec(_spec); _spec.loader.exec_module(_m33)
model = _m33.VCellModel(feats, P=P).to(DEV)
model.set_strain_avg()
print(f"[模型] 参数 {sum(p.numel() for p in model.parameters())/1e6:.2f}M (v3.3 遗传相似度+Morgan+ESM2)", flush=True)
...
def evaluate(ep):
    ...
    for scene in ['val_chem_only', 'val_strain_only', 'val_both', 'val_time']:
        ...
        with torch.no_grad():
            xd = {}
            for k, v in x.items():
                if k in ('ctx_prior', 'chem_morgan', 'strain_sim'):
                    xd[k] = v.to(DEV)
                else:
...
for ep in range(1, EPOCHS + 1):
    ...
    for i in pbar:
        ...
        x_gpu = {k: (v.to(DEV) if k in ('ctx_prior', 'chem_morgan', 'strain_sim') else [t.to(DEV) for t in v]) for k, v in x.items()}
        pred = model(x_gpu)
        ...
        if is_treat.any():
            pos = np.where(is_treat)[0]
            yB, yS, yC, yT = model.components({k: (v[pos].to(DEV) if k in ('ctx_prior', 'chem_morgan', 'strain_sim') else [t[pos].to(DEV) for t in v])
    ...
    if ep % 10 == 0 or ep == EPOCHS:
        evaluate(ep)
        torch.save(model.state_dict(), f"{DATA}/model_v33.pt")
    if avg_loss < best_score:
        best_score = avg_loss
        torch.save(model.state_dict(), f"{DATA}/model_v33_best.pt")
