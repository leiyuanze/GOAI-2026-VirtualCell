# -*- coding: utf-8 -*-
"""
GOAI 虚拟细胞 · 05 训练 v3.4（T分支增强 + unseen门控归零）
...
print(f"[模型] 参数 {sum(p.numel() for p in model.parameters())/1e6:.2f}M (v3.4 遗传加权基线+Morgan+ESM2)", flush=True)
...
for ep in range(1, EPOCHS + 1):
    ...
    if ep % 10 == 0 or ep == EPOCHS:
        evaluate(ep)
        torch.save(model.state_dict(), f"{DATA}/model_v34.pt")
    if avg_loss < best_score:
        best_score = avg_loss
        torch.save(model.state_dict(), f"{DATA}/model_v34_best.pt")
