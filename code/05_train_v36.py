# -*- coding: utf-8 -*-
"""
GOAI 虚拟细胞 · 05 训练 v3.6（T分支增强 + unseen门控归零）
...
_spec = importlib.util.spec_from_file_location("m30", r"D:\leiyuanze\Datawhale\AI for Research\虚拟细胞\vcell\04_model_v30.py")
_m30 = importlib.util.module_from_spec(_spec); _spec.loader.exec_module(_m30)
model = _m30.VCellModel(feats, P=P).to(DEV)
model.set_strain_avg()
print(f"[模型] 参数 {sum(p.numel() for p in model.parameters())/1e6:.2f}M (v3.6 蛋白先验强权重(0.3+0.15))", flush=True)
...
for ep in range(1, EPOCHS + 1):
    ...
    for i in pbar:
        ...
        loss += 0.3 * loss_prot + 0.15 * loss_graph
        optimizer.zero_grad()
        loss.backward()
    ...
    if ep % 10 == 0 or ep == EPOCHS:
        evaluate(ep)
        torch.save(model.state_dict(), f"{DATA}/model_v36.pt")
    if avg_loss < best_score:
        best_score = avg_loss
        torch.save(model.state_dict(), f"{DATA}/model_v36_best.pt")
