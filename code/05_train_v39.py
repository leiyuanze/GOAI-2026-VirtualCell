# -*- coding: utf-8 -*-
"""
GOAI 虚拟细胞 · 05 训练 v3.9（T分支增强 + unseen门控归零）
...
print(f"[模型] 参数 {sum(p.numel() for p in model.parameters())/1e6:.2f}M (v3.9 GO通路+PPI图正则+蛋白先验)", flush=True)
...
# ★ 5.4.4 PPI 图正则：物理互作蛋白协同响应（相连蛋白预测差异小）
def ppi_reg_loss(pred, ppi_i, ppi_j):
    diff = pred[:, ppi_i] - pred[:, ppi_j]  # (batch, E)
    return (diff ** 2).mean()
...
# ★ 加载 PPI 边（采样 2 万条降低计算成本）
ppi_edges = feats['ppi_edges']
if len(ppi_edges) > 20000:
    rng = np.random.RandomState(42)
    ppi_edges = ppi_edges[rng.choice(len(ppi_edges), 20000, replace=False)]
ppi_i = torch.tensor(ppi_edges[:, 0], device=DEV, dtype=torch.long)
ppi_j = torch.tensor(ppi_edges[:, 1], device=DEV, dtype=torch.long)
print(f"[预计算] PPI 边 {len(ppi_edges)} 条（采样后）", flush=True)
...
for ep in range(1, EPOCHS + 1):
    ...
    for i in pbar:
        ...
        # ★ 逐蛋白 corr loss（0.1）+ 蛋白先验正则（0.05）+ PPI 图正则（0.05）
        loss_prot = per_protein_corr_loss(pred, yt, m)
        loss_graph = graph_reg_loss(pred, m, V_graph_t)
        loss_ppi = ppi_reg_loss(pred, ppi_i, ppi_j)
        loss += 0.1 * loss_prot + 0.05 * loss_graph + 0.05 * loss_ppi
        optimizer.zero_grad()
        loss.backward()
    ...
    if ep % 10 == 0 or ep == EPOCHS:
        evaluate(ep)
        torch.save(model.state_dict(), f"{DATA}/model_v39.pt")
    if avg_loss < best_score:
        best_score = avg_loss
        torch.save(model.state_dict(), f"{DATA}/model_v39_best.pt")
