# -*- coding: utf-8 -*-
"""
GOAI 虚拟细胞 · 05 训练 v4.1（T分支增强 + unseen门控归零）
...
_spec = importlib.util.spec_from_file_location("m41", r"D:\leiyuanze\Datawhale\AI for Research\虚拟细胞\vcell\04_model_v41.py")
_m41 = importlib.util.module_from_spec(_spec); _spec.loader.exec_module(_m41)
model = _m41.VCellModel(feats, P=P).to(DEV)
model.set_strain_avg()
print(f"[模型] 参数 {sum(p.numel() for p in model.parameters())/1e6:.2f}M (v4.1 GO通路(unseen门控)+蛋白先验)", flush=True)
...
for ep in range(1, EPOCHS + 1):
    ...
    if ep % 10 == 0 or ep == EPOCHS:
        evaluate(ep)
        torch.save(model.state_dict(), f"{DATA}/model_v41.pt")
    if avg_loss < best_score:
        best_score = avg_loss
        torch.save(model.state_dict(), f"{DATA}/model_v41_best.pt")
