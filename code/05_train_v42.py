# -*- coding: utf-8 -*-
"""
GOAI 虚拟细胞 · 05 训练 v4.2（修正 PPI Δ 协同正则）
基于 v3.7（GO 通路注意力 + 蛋白先验），新增：
  - PPI 图正则 v2：约束「Δ 协同」(Δ_i - Δ_j)² 而非绝对丰度
    （v3.9 失败根因：约束 pred 绝对丰度，基线差异主导；Δ 才对应协同响应）
  - 边取 score>=900 的 top 5000，权重 0.01（v3.9 用 20000 边 + 0.05 权重过强）
"""
...
print(f"[模型] 参数 {sum(p.numel() for p in model.parameters())/1e6:.2f}M (v4.2 修正PPI Δ协同正则)", flush=True)
...
# ★ PPI Δ 协同正则（修正版）：约束预测 Δ 在 PPI 边上的协同
# Δ_pred = pred - matched_control；同边蛋白 Δ 应接近
def ppi_delta_loss(pred, m, ppi_i, ppi_j, yc):
    # pred: (B, P); yc: (B, P) matched control；仅对处理样本用
    delta = pred - yc  # (B, P)
    d_i = delta[:, ppi_i]  # (B, E)
    d_j = delta[:, ppi_j]
    diff = (d_i - d_j) ** 2
    m_b = m.bool()
    m_i = m_b[:, ppi_i]; m_j = m_b[:, ppi_j]
    mask_e = (m_i & m_j).float()  # 边两端都有观测
    denom = mask_e.sum().clamp(min=1)
    return (diff * mask_e).sum() / denom
...
# ★ PPI 边（top 5000，score>=900 里取前 5000）
...
print(f"[预计算] PPI 边 {len(ppi_edges)} 条 (top5000, score>=900)", flush=True)

print("[预计算] 蛋白共表达主成分 V ...", flush=True)
...
print(f"[预计算] V_graph {V_graph.shape} 完成", flush=True)
...
for ep in range(1, EPOCHS + 1):
    ...
    for i in pbar:
        ...
        if is_treat.any():
            ...
            # ★ PPI Δ 协同（仅处理样本，yc 有值的）
            ppi_ok = torch.isfinite(yc).any(dim=1)
            if ppi_ok.any():
                yc_pp = torch.where(torch.isfinite(yc), yc, torch.zeros_like(yc))
                loss_ppi_val = ppi_delta_loss(pred[pos][ppi_ok], m[pos][ppi_ok], ppi_i, ppi_j, yc_pp[ppi_ok])
                loss += 0.01 * loss_ppi_val
        if is_treat.any():
            pos = np.where(is_treat)[0]
        ...
        pbar.set_postfix(loss=f"{loss.item():.4f}", mse=f"{loss_mse.item():.4f}", ppi=f"{loss_ppi_val.item():.4f}")
    avg_loss = total_loss / max(n_batch, 1)
    scheduler.step(avg_loss)
    lr_now = optimizer.param_groups[0]['lr']
    if ep % 10 == 0 or ep == EPOCHS:
        evaluate(ep)
        torch.save(model.state_dict(), f"{DATA}/model_v42.pt")
    if avg_loss < best_score:
        best_score = avg_loss
        torch.save(model.state_dict(), f"{DATA}/model_v42_best.pt")
