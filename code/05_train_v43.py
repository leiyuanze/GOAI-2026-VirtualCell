# -*- coding: utf-8 -*-
"""
GOAI 虚拟细胞 · 05 训练 v4.3（排序 loss + Δ 一致性强化，教程 5.6.3 技巧2）
...
print(f"[模型] 参数 {sum(p.numel() for p in model.parameters())/1e6:.2f}M (v4.3 排序loss+Δ一致性)", flush=True)
...
# ★ 排序 loss（教程 5.6.3 技巧2）：按 |Δ| 排序对齐
# 对每个样本：预测的 |Δ| 排序 与 真实的 |Δ| 排序 应一致（Spearman 近似的可微版）
# 实现：对 batch 内样本的 |Δ| 向量做中心化相关（等价于对排序敏感的 PCC）
def rank_loss(pred, yt, m, yc):
    yc = yc.to(pred.device)
    # Δ 只在处理样本有定义；yc 有值处才参与
    dp = torch.where(torch.isfinite(yc), pred - yc, torch.zeros_like(yc))
    dt = torch.where(torch.isfinite(yc), yt - yc, torch.zeros_like(yc))
    ap = dp.abs(); at = dt.abs()
    v = m.bool() & torch.isfinite(yc)
    # 逐样本：预测 |Δ| 与真实 |Δ| 的相关（排序结构的连续代理）
    # 用中心化相关，但只保留有观测的蛋白
    valid = v.any(dim=1)
    if valid.sum() == 0:
        return torch.tensor(0.0, device=pred.device)
    # 处理每个样本（batch 内样本数不多，逐样本可接受）
    losses = []
    for i in range(pred.shape[0]):
        ...
        if vi.sum() < 10:
            continue
        a = ap[i][vi] - ap[i][vi].mean()
        b = at[i][vi] - at[i][vi].mean()
...
for ep in range(1, EPOCHS + 1):
    ...
    if ep % 10 == 0 or ep == EPOCHS:
        evaluate(ep)
        torch.save(model.state_dict(), f"{DATA}/model_v43_{_SEED}.pt")
    if avg_loss < best_score:
        best_score = avg_loss
        torch.save(model.state_dict(), f"{DATA}/model_v43_{_SEED}_best.pt")
