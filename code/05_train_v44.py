# -*- coding: utf-8 -*-
"""
GOAI 虚拟细胞 · 05 训练 v4.4（排序 loss + 残差形状监督，评测感知训练）
基于 v3.7（GO通路注意力+蛋白先验），新增两个教程级 loss：
  1) 排序 loss（权重 0.05）：按 |Δ| 排序对齐——直接打 M6 DEP + M1 形状
  2) Δ 一致性 loss 强化（权重 0.2）：显式对齐 Δ_pred 与 Δ_true 的 FC 方向
关键：M6 DEP 5% 与 M1 FC 25% 都围绕「高效应蛋白是否被正确排序」，
之前所有版本都只做 MSE+corr，从未直接优化排序结构——这是真正的空白点。
"""
...
print(f"[模型] 参数 {sum(p.numel() for p in model.parameters())/1e6:.2f}M (v4.4 排序loss+残差形状监督)", flush=True)
...
# ★ 排序 loss（教程 5.6.3 技巧2）：按 |Δ| 排序对齐（向量化全局版）
# 评测 M1/M6 都是「全局展开」算 PCC——预测 Δ 与真值 Δ 的元素级相关。
# 排序 loss 的向量化实现：对 |Δ| 展平后做中心化相关，
# 它直接奖励「高效应蛋白排序一致」，与 M6 DEP 的方向准确率对齐。
def rank_loss(pred, yt, m, yc):
    yc = yc.to(pred.device)
    dp = torch.where(torch.isfinite(yc), pred - yc, torch.zeros_like(yc))
    dt = torch.where(torch.isfinite(yc), yt - yc, torch.zeros_like(yc))
    ap = dp.abs(); at = dt.abs()
    v = m.bool() & torch.isfinite(yc)
    # 全局展平（与评测口径一致：M1/M6 均全局展开算 PCC）
    a = ap[v]; b = at[v]
    if a.numel() < 10:
        return torch.tensor(0.0, device=pred.device)
...
# ★ 技巧2：逐蛋白 corr loss（提升谱形状，mask-aware）
...
# ★ 残差形状 loss（评测感知，v4.4 核心创新）：
# 评测 M3/M4 测的是 PCC(Δ_pred − μ_ctx, Δ_true − μ_ctx) / PCC(Δ_pred − μ_drug, Δ_true − μ_drug)
# 之前所有版本只监督分支组件（yC/yT），从未对「最终输出的残差形状」直接监督。
# 这里把评测公式本身变成训练 loss：对 batch 内处理样本的 Δ_pred 做残差化后算 PCC。
def resid_shape_loss(pred, yt, m, yc, mu):
    """PCC((Δ_pred − μ), (Δ_true − μ))，mu 为 LOO μ_ctx 或 μ_drug（真值常数）"""
    yc = yc.to(pred.device); mu = mu.to(pred.device)
    dp = torch.where(torch.isfinite(yc), pred - yc, torch.zeros_like(yc))
    dt = torch.where(torch.isfinite(yc), yt - yc, torch.zeros_like(yc))
    rp = dp - mu
    rt = dt - mu
    v = m.bool() & torch.isfinite(yc) & torch.isfinite(mu)
    if v.sum() < 10:
        return torch.tensor(0.0, device=pred.device)
    a = rp[v]; b = rt[v]
    a = a - a.mean(); b = b - b.mean()
...
# ★ 5.4.4：蛋白先验正则（协方差低秩约束：输出应在真实共表达主成分空间）
...
K_GRAPH = 64
svd = TruncatedSVD(n_components=K_GRAPH, random_state=42)
svd.fit(Xc_graph)
V_graph = svd.components_.T.astype(np.float32)  # (P, 64)
V_graph = V_graph / (np.linalg.norm(V_graph, axis=0, keepdims=True) + 1e-8)
V_graph_t = torch.tensor(V_graph, device=DEV)
print(f"[预计算] 蛋白共表达主成分 V {V_graph.shape} 完成", flush=True)
    ...
    for i in pbar:
        ...
        if is_treat.any():
            ...
            if fc_ok.any():
                ...
                # ★ 排序 loss（权重 0.05，教程 5.6.3 技巧2）
                loss_rank_val = rank_loss(pred[pos][fc_ok], yt[pos][fc_ok], m[pos][fc_ok], yc[fc_ok])
                loss += 0.05 * loss_rank_val
        # 组件级监督
        ...
        if is_treat.any():
            ...
            # ★ 残差形状监督（v4.4 核心）：直接对最终输出的残差形状做 M3/M4 对齐
            yc = y_ctrl_all_t[b[pos]]
            resid_ok = torch.isfinite(yc).any(dim=1)
            if resid_ok.any():
                loss_resid_ctx_val = resid_shape_loss(pred[pos][resid_ok], yt[pos][resid_ok],
                                                      m[pos][resid_ok], yc[resid_ok],
                                                      mu_ctx_t[b[pos]][resid_ok])
                loss_resid_drug_val = resid_shape_loss(pred[pos][resid_ok], yt[pos][resid_ok],
                                                       m[pos][resid_ok], yc[resid_ok],
                                                       mu_drug_t[b[pos]][resid_ok])
                loss += 0.15 * loss_resid_ctx_val + 0.15 * loss_resid_drug_val
        # ★ 逐蛋白 corr loss（0.1）+ 蛋白先验正则（0.05）
        ...
        pbar.set_postfix(loss=f"{loss.item():.4f}", mse=f"{loss_mse.item():.4f}", fc=f"{loss_fc_val.item():.3f}", rank=f"{loss_rank_val.item():.3f}")
    avg_loss = total_loss / max(n_batch, 1)
    scheduler.step(avg_loss)
    lr_now = optimizer.param_groups[0]['lr']
    if ep % 10 == 0 or ep == EPOCHS:
        evaluate(ep)
        torch.save(model.state_dict(), f"{DATA}/model_v44_{_SEED}.pt")
    if avg_loss < best_score:
        best_score = avg_loss
        torch.save(model.state_dict(), f"{DATA}/model_v44_{_SEED}_best.pt")
