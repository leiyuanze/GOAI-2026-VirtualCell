# -*- coding: utf-8 -*-
"""
GOAI 虚拟细胞 · 05 训练 v4.7（v37 + pairwise排序loss单变量）
...
print(f"[模型] 参数 {sum(p.numel() for p in model.parameters())/1e6:.2f}M (v4.7 GO通路+pairwise排序)", flush=True)
...
# ★ pairwise 排序 loss（v4.7，教程 5.6.3 技巧2 的正确实现）
# 采样蛋白对：|Δ_true_i| > |Δ_true_j| 的样本对，要求 |Δ_pred_i| > |Δ_pred_j|
# hinge 形式，直接优化 M6 DEP 高效应蛋白检出
def rank_pair_loss(pred, yt, m, yc, n_pairs=512, margin=0.05):
    ...
    at_a = at[a[:, 0], a[:, 1]]; at_b = at[b[:, 0], b[:, 1]]
    ap_a = ap[a[:, 0], a[:, 1]]; ap_b = ap[b[:, 0], b[:, 1]]
    diff_true = at_a - at_b
    keep = diff_true.abs() > 1e-4
    if keep.sum() < 10:
        return torch.tensor(0.0, device=pred.device)
    ...
    return torch.relu(margin - diff_pred).mean()
...
for ep in range(1, EPOCHS + 1):
    ...
    for i in pbar:
        ...
        if is_treat.any():
            ...
            if fc_ok.any():
                ...
                # ★ pairwise 排序 loss（0.05，M6 DEP 高效应蛋白排序）
    ...
    if ep % 10 == 0 or ep == EPOCHS:
        evaluate(ep)
        torch.save(model.state_dict(), f"{DATA}/model_v47_{_SEED}.pt")
    if avg_loss < best_score:
        best_score = avg_loss
        torch.save(model.state_dict(), f"{DATA}/model_v47_{_SEED}_best.pt")
