...
_spec = importlib.util.spec_from_file_location("m37", r"D:\leiyuanze\Datawhale\AI for Research\虚拟细胞\vcell\04_model_v49.py")
...
print(f"[模型] 参数 {sum(p.numel() for p in model.parameters())/1e6:.2f}M (v4.9 门控修正+指纹噪声)", flush=True)
...
for ep in range(1, EPOCHS + 1):
    ...
    if ep % 10 == 0 or ep == EPOCHS:
        evaluate(ep)
        torch.save(model.state_dict(), f"{DATA}/model_v49_{_SEED}.pt")
    if avg_loss < best_score:
        best_score = avg_loss
        torch.save(model.state_dict(), f"{DATA}/model_v49_{_SEED}_best.pt")
