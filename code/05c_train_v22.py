# -*- coding: utf-8 -*-
"""
GOAI 虚拟细胞 · 05c 训练 v2.2（正则化导向，防过拟合）
增量 vs v2.1：
  1) 实体 Dropout（strain/chem embedding 随机置零，p=0.15）
  2) 解耦一致性正则：中性菌株/中性化合物变体输出对齐「主输出 − 对应残差贡献」
  3) 模型选择防过拟合：多场景加权（val_both×0.4 + val_chem×0.3 + val_time×0.3 的超越 control 幅度）
  4) 消融开关：--no_decouple / --no_drop 可关闭
"""
...
# ---------- 特征（全量预取到 GPU，训练时索引切片，避免逐 batch 拷贝）----------
...
model = _m04.VCellModel(feats, P=P, drop_ent=0.15 if USE_DROP else 0.0).to(DEV)
...
for ep in range(1, EPOCHS + 1):
    ...
    for i in pbar:
        ...
        # 解耦一致性正则（处理样本，latent 空间；每 2 batch 一次降开销）
        if USE_DECOUPLE and is_treat.any() and (i // BATCH) % 2 == 0:
            pos = np.where(is_treat)[0]
            xp = {k: [t[pos] for t in v] for k, v in x.items()}
    ...
    if ep % 10 == 0 or ep == EPOCHS:
        ...
        if score > best_score:
            best_score = score
            torch.save(model.state_dict(), f"{DATA}/model_v22_best.pt")
    torch.save(model.state_dict(), f"{DATA}/model_v22_last.pt")
print("\n训练完成", flush=True)
