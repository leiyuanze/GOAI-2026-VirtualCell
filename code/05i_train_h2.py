...
_spec = importlib.util.spec_from_file_location("model04", r"D:\leiyuanze\Datawhale\AI for Research\虚拟细胞\vcell\04_model_h2.py")
_m04 = importlib.util.module_from_spec(_spec); _spec.loader.exec_module(_m04)
model = _m04.VCellModel(feats, P=P, latent=64, drop_ent=0.15 if USE_DROP else 0.0).to(DEV)
print(f"[模型] 参数 {sum(p.numel() for p in model.parameters())/1e6:.2f}M | latent=64", flush=True)
