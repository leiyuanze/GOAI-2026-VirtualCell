# -*- coding: utf-8 -*-
"""
GOAI 虚拟细胞 · 05k 训练 v2.4（教程 5.6 提分项全落地）
基于 v2.1 架构（04_model_v21.py），新增：
...
print(f"[模型] 参数 {sum(p.numel() for p in model.parameters())/1e6:.2f}M (v2.1 架构)", flush=True)
...
def protein_corr_loss(pred, yt, m):
    """蛋白间相关性 loss（mask-aware，抽高方差子集）—— 教程 5.6.3 技巧4"""
    n = m.sum(0)
    keep = n >= 20
    if keep.sum() < 50:
        return torch.tensor(0.0, device=pred.device)
    ...
    syt = torch.sqrt((dyt ** 2).sum(0)).clamp(min=1e-8); spr = torch.sqrt((dpr ** 2).sum(0)).clamp(min=1e-8)
    zt = dyt / syt; zp = dpr / spr
    # 蛋白相关矩阵（近似：共同观测简化——用中心化后向量内积）
    Ct = zt.T @ zt / n2.clamp(min=1)
    Cp = zp.T @ zp / n2.clamp(min=1)
    return ((Cp - Ct) ** 2).mean()
...
for ep in range(1, EPOCHS + 1):
    ...
    if ep % 10 == 0 or ep == EPOCHS:
        ...
        if score > best_score:
            best_score = score
            torch.save(model.state_dict(), f"{DATA}/model_v24_best.pt")
    torch.save(model.state_dict(), f"{DATA}/model_v24_last.pt")
print("\n训练完成", flush=True)
