# -*- coding: utf-8 -*-
"""
GOAI 虚拟细胞 · 05n 训练 v2.6（T分支增强 + unseen门控归零）
...
_spec = importlib.util.spec_from_file_location("m26", r"D:\leiyuanze\Datawhale\AI for Research\虚拟细胞\vcell\04_model_v26.py")
_m26 = importlib.util.module_from_spec(_spec); _spec.loader.exec_module(_m26)
model = _m26.VCellModel(feats, P=P).to(DEV)
print(f"[模型] 参数 {sum(p.numel() for p in model.parameters())/1e6:.2f}M (v2.6 T分支增强)", flush=True)
...
for ep in range(1, EPOCHS + 1):
    ...
    if ep % 10 == 0 or ep == EPOCHS:
        evaluate(ep)
        torch.save(model.state_dict(), f"{DATA}/model_v26.pt")
    if avg_loss < best_score:
        best_score = avg_loss
        torch.save(model.state_dict(), f"{DATA}/model_v26_best.pt")
    print(f"  avg_loss={avg_loss:.4f}  lr={optimizer.param_groups[0]['lr']:.2e}  best={best_score:.4f}", flush=True)
