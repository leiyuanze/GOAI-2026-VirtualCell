...
_spec = importlib.util.spec_from_file_location("m21", r"D:\leiyuanze\Datawhale\AI for Research\虚拟细胞\vcell\04_model_v21.py")
_m21 = importlib.util.module_from_spec(_spec); _spec.loader.exec_module(_m21)

def load_model(path):
    m = _m21.VCellModel(feats, P=P)
    m.load_state_dict(torch.load(f"{DATA}/{path}", map_location=DEV, weights_only=True))
    return m.to(DEV).eval()

models = [load_model(p) for p in MODELS]
...
sub.to_csv(f"{DATA}/prediction_ensemble.csv")
assert len(sub) == 4454 and sub.shape[1] == 4422
assert not sub.isna().any().any() and np.isfinite(sub.values).all()
print("\n[提交] prediction_ensemble.csv 生成并校验通过（4,454 × 4,422，无 NA/inf）", flush=True)
