# -*- coding: utf-8 -*-
"""
GOAI 虚拟细胞 · 07 三模型集成（TTA，教程 5.6.3 技巧5）
seed 42/43/44 平均预测 → val 四场景评估 + test 提交
"""
...
MODELS = ['model_v21.pt', 'model_v21_s43.pt', 'model_v21_s44.pt']
...
_spec21 = importlib.util.spec_from_file_location("m21", r"D:////leiyuanze////Datawhale////AI for Research////虚拟细胞////vcell////04_model_v21.py")
_m21 = importlib.util.module_from_spec(_spec21); _spec21.loader.exec_module(_m21)
_spec25 = importlib.util.spec_from_file_location("m25", r"D:////leiyuanze////Datawhale////AI for Research////虚拟细胞////vcell////04_model_v25.py")
_m25 = importlib.util.module_from_spec(_spec25); _spec25.loader.exec_module(_m25)
...
models = [load_model('model_v21.pt', _m21.VCellModel), load_model('model_v21_s43.pt', _m21.VCellModel),
          load_model('model_v21_s44.pt', _m21.VCellModel), load_model('model_v25_best.pt', _m25.VCellModel)]
print(f"[集成] {len(models)} 模型加载完成", flush=True)

def make_x(idx):
    return {'bio': [torch.from_numpy(feats['strain_id'][idx]), torch.from_numpy(feats['chem_id'][idx]),
...
def ensemble_pred(idx):
    ...
    with torch.no_grad():
        for m in models:
            preds.append(m({k: [t.to(DEV) for t in v] for k, v in x.items()}).cpu().numpy())
    return np.mean(preds, axis=0)

# ---------- matched control（评估 FC PCC 用）----------
...
# ---------- val 评估 ----------
print(f"\n[集成评估] {'场景':<16}{'样本':>5}{'RMSE':>7}{'GlobalR2':>9}{'蛋白R2中位':>10}{'FC PCC':>8}", flush=True)
...
x = {'bio': [torch.from_numpy(tstrain), torch.from_numpy(tchem), torch.from_numpy(tchash), torch.from_numpy(tmed),
             torch.from_numpy(ttemp), torch.from_numpy(ttfeat), torch.from_numpy(tsm), torch.from_numpy(tct)],
...
sub.to_csv(f"{DATA}/prediction_ensemble4.csv")
assert len(sub) == 4454 and sub.shape[1] == 4422
assert not sub.isna().any().any() and np.isfinite(sub.values).all()
print("\n[提交] prediction_ensemble4.csv 生成并校验通过（4,454 × 4,422，无 NA/inf）", flush=True)
