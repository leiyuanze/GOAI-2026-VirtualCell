# -*- coding: utf-8 -*-
"""
GOAI 虚拟细胞 · 07f 提交 v3.1：3×v2.1(s42/43/44) + v3.1(Morgan+ESM2)
评估 + 提交，生成 prediction_ensemble7.csv (4454x4422)
"""
...
models = [
    ...
    (load_model('model_v30_best.pt', _m30.VCellModel, set_avg=True), 'v30'),
]
print(f"[集成] {len(models)} 模型加载完成 (3×v2.1 + v3.1)", flush=True)
...
def make_x(idx, tag):
    ...
    if tag == 'v30':
        ...
        d['chem_morgan'] = torch.from_numpy(morgan_all[idx])
    return d
...
def ensemble_pred(idx):
    ...
    with torch.no_grad():
        for m, tag in models:
            x = make_x(idx, tag)
            preds.append(m(to_gpu(x)).cpu().numpy())
...
# ---------- val ----------
print(f"\n[新集成 3×v2.1+v3.1] {'场景':<16}{'样本':>5}{'RMSE':>7}{'GlobalR2':>9}{'蛋白R2中位':>10}{'FC PCC':>8}", flush=True)
...
# test 的 Morgan 指纹（从 train_val 的 perturbation -> morgan64 映射取，缺失用零）
pert2morgan = {}
...
for i in np.where(train_mask_arr)[0]:
    k = ctx_key_tr[i]
    ctx_grp.setdefault(k, []).append(tr_y_nan[i])
