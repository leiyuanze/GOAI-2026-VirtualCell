# -*- coding: utf-8 -*-
"""迁移融合 v3（v50ens 版，8/16）：模型预测 + 化学结构迁移预测 加权融合
alpha=0.2（由 _mig_fusion 网格搜索确定，M1 +0.006 / RMSE -0.011）
仅对 test_chem_only / test_both（新化合物场景）融合；strain/time 用原模型
★ 合规修复：迁移池与对照池仅用 train 划分（原 08 用全量对照池，含 val 真值）
输入：prediction_v50ens_base.csv → 输出：prediction_v50ens.csv
"""
...
out.to_csv(f'{DATA}/prediction_v50ens.csv')
print(f'[提交] prediction_v50ens.csv 生成 {out.shape}, alpha={ALPHA}')
