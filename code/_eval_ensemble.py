# -*- coding: utf-8 -*-
"""
v37 + v5.0 集成评估（模型平均，检验架构互补性）
用法：python _eval_ensemble.py [w_v37] [w_v50]
"""
...
W37 = float(sys.argv[1]) if len(sys.argv) > 1 else 0.5
W50 = float(sys.argv[2]) if len(sys.argv) > 2 else 0.5
...
m37.load_state_dict(torch.load(f"{DATA}/model_v37_42_best.pt", map_location=DEV))
m37.set_strain_avg(); m37.eval()
m50 = _m50.VCellModel(feats, P=P, response_basis=U_basis).to(DEV)
m50.load_state_dict(torch.load(f"{DATA}/model_v50_42_best.pt", map_location=DEV))
m50.set_strain_avg(); m50.eval()

print(f"=== 集成 {W37}*v37 + {W50}*v50 ===")
...
for scene in ['val_chem_only', 'val_strain_only', 'val_both', 'val_time']:
    ...
    x = make_x(idx)
    with torch.no_grad():
        pred = W37 * m37(x).cpu().numpy() + W50 * m50(x).cpu().numpy()
