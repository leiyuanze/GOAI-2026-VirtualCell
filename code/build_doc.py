...
doc.add_paragraph(
    ...
    '③v5.x 接手重构——先完成数据泄漏审计与修复（3 处统计量越界全部改为 train-only），'
...
doc.add_paragraph(
    '三个核心设计原则。其一，结构对齐——模型组��与评测维度一一对应：'
...
doc.add_paragraph(
    '阶段三（v5.x 接手重构，8/16）：①P0 泄漏审计发现并修复 3 处统计量越界'
...
doc.add_paragraph(
    ...
    '三阶段训练（gpt2 方案）：阶段 A 对照模型预训练（仅 L_ctrl，40 epoch）→ '
